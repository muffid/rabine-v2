    ...
    return (
        <div id='doa' className='w-full bg-white p-6 flex flex-col items-center gap-y-2 px-8 text-[#575757]'>
            ...
            <textarea
                    ...
                    />
                    <button onClick={()=>postComment()} className='text-white fadeUp flex flex-row items-center justify-center text-sm rounded-lg gap-2 border bg-[#151313]  px-3 py-2 my-8
                                        hover:bg-white fadeUp hover:text-black hover:scale-105 transition-all ease-out'>
                            <FaComment className=''/> Kirim Ucapan
                    </button>
            <div className='fadeUp w-full flex flex-col items-center justify-start space-y-3  rounded-xl p-4 text-black text-sm max-h-[600px] lg:max-h-[700px] overflow-y-scroll'>
                {comments.map(comment => (
                    <div key={comment.Comment_Id} className='w-full flex flex-col items-start justify-start space-y-4 bg-slate-100/70 text-slate-900 rounded-lg py-4 px-8'>
