...
export default function Comments({slug,url,tamu}) {
    ...
    return (
        <div id='doa' className='w-full  flex flex-col items-center justify-center gap-y-2  text-white px-14 py-16'>
            ...
            <div className='p-6 flex flex-col items-center gap-y-2  bg-white rounded-lg w-full'>
                ...
                <textarea
                        ...
                        />
                        <button onClick={()=>postComment()}  className='text-[#222831] fadeUp flex flex-row items-center justify-center text-sm rounded-lg gap-2  bg-white px-6 py-2 mt-4 mb-8
            ...
            <div className='fadeUp w-full flex flex-col items-center justify-start space-y-3  rounded-xl text-black text-sm max-h-[600px] lg:max-h-[700px] overflow-y-scroll'>
                {comments.map(comment => (
                    <div key={comment.Comment_Id} className='w-full flex flex-col items-start justify-start space-y-4 bg-[#4A4544] text-slate-900 rounded-lg py-4 px-8'>
                        <div className='flex flex-row items-start gap-x-3 text-white'>
                            <div className='w-[5%]'>
                              <FaRegUserCircle className='text-lg'/>
                            </div>
                            <div className='flex-col items-start justify-start'>
                                <div className='flex flex-row items-start  '><p className='font-bold  text-white'>{comment.Comment_User}</p></div>
                                <p className='text-white my-1'>{comment.Comment_Content}</p>
                                <div className='flex flex-row items-start justify-start gap-x-2 text-xs text-slate-100'>
                    ...
                    <>
                        <input
                            ...
                            value={replyToPost}
                            className="border-b-[1px] border-slate-700 text-white bg-transparent outline-none focus:border-slate-600 text-sm w-4/5 p-2 ml-10"
                        ...
                        {comment.replies && comment.replies.map(reply => (
                             ...
                             <div className='w-[5%] mt-1 text-white'>
                              <FaRegUserCircle className='text-md'/>
                            </div>
                             <div className='flex-col items-start '>
                             <div  className='flex flex-col items-start justify-start '>
                                <p className='font-bold  text-white'>{reply.Comment_User}</p>
                                <p className='text-slate-100 my-1'>{reply.Comment_Content}</p>
